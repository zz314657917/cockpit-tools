...
static WORKBUDDY_INSTANCE_STORE_LOCK: std::sync::LazyLock<Mutex<()>> =
    std::sync::LazyLock::new(|| Mutex::new(()));

const WORKBUDDY_INSTANCES_FILE: &str = "workbuddy_instances.json";

fn instances_path() -> Result<PathBuf, String> {
    let data_dir = modules::account::get_data_dir()?;
    Ok(data_dir.join(WORKBUDDY_INSTANCES_FILE))
}

pub fn load_instance_store() -> Result<InstanceStore, String> {
    let path = instances_path()?;
    instance_store::load_instance_store(&path, WORKBUDDY_INSTANCES_FILE)
}

pub fn save_instance_store(store: &InstanceStore) -> Result<(), String> {
    let path = instances_path()?;
    instance_store::save_instance_store(&path, WORKBUDDY_INSTANCES_FILE, store)
}
...
pub fn update_default_settings(
    ...
    let _lock = WORKBUDDY_INSTANCE_STORE_LOCK
        .lock()
        .map_err(|_| "无法获取实例锁")?;
...
pub fn get_instance_defaults() -> Result<InstanceDefaults, String> {
    ...
    let default_user_data_dir = get_default_workbuddy_user_data_dir()?;
    Ok(InstanceDefaults {
        ...
        if user_dir_path.exists() {
            ...
            if has_entries {
                ...
                modules::logger::log_info(&format!(
                    "[WorkBuddy Instance] 复制来源实例需要空目录，但目标已存在：{}",
                    resolved_path
                ));
