...
static CODEBUDDY_INSTANCE_STORE_LOCK: std::sync::LazyLock<Mutex<()>> =
...
pub fn load_instance_store() -> Result<InstanceStore, String> {
    ...
    instance_store::load_instance_store(&path, CODEBUDDY_INSTANCES_FILE)
}

pub fn save_instance_store(store: &InstanceStore) -> Result<(), String> {
    let path = instances_path()?;
    instance_store::save_instance_store(&path, CODEBUDDY_INSTANCES_FILE, store)
}
...
pub fn update_default_settings(
    ...
    let _lock = CODEBUDDY_INSTANCE_STORE_LOCK
        .lock()
        .map_err(|_| "无法获取实例锁")?;
    let mut store = load_instance_store()?;
    let settings = &mut store.default_settings;

    // CodeBuddy 实例不支持“跟随当前账号”，直接忽略 follow_local_account。
...
pub fn get_default_codebuddy_user_data_dir() -> Result<PathBuf, String> {
    if let Some(path) = modules::codebuddy_account::get_default_codebuddy_data_dir() {
        return Ok(path);
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        return Ok(home.join("Library/Application Support/CodeBuddy"));
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        return Ok(PathBuf::from(appdata).join("CodeBuddy"));
    }

    #[cfg(target_os = "linux")]
    {
        let home = dirs::home_dir().ok_or("无法获取用户主目录")?;
        return Ok(home.join(".config/CodeBuddy"));
    }
...
pub fn get_instance_defaults() -> Result<InstanceDefaults, String> {
    ...
    let default_user_data_dir = get_default_codebuddy_user_data_dir()?;
    Ok(InstanceDefaults {
...
pub fn create_instance(params: CreateInstanceParams) -> Result<InstanceProfile, String> {
    ...
    if use_existing_dir {
        ...
        let source_dir = match params.copy_source_instance_id.as_deref() {
            Some("__default__") | None => get_default_codebuddy_user_data_dir()?,
            Some(source_id) => {
        ...
        if user_dir_path.exists() {
            ...
            if has_entries {
                ...
                modules::logger::log_info(&format!(
                    "[CodeBuddy Instance] 复制来源实例需要空目录，但目标已存在: {}",
                    resolved_path
                ));
