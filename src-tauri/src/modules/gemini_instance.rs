...
static GEMINI_INSTANCE_STORE_LOCK: std::sync::LazyLock<Mutex<()>> =
    std::sync::LazyLock::new(|| Mutex::new(()));

const GEMINI_INSTANCES_FILE: &str = "gemini_instances.json";
...
fn instances_path() -> Result<PathBuf, String> {
    ...
    Ok(data_dir.join(GEMINI_INSTANCES_FILE))
}

fn gemini_home_from_cli_root(cli_home_root: &Path) -> PathBuf {
    cli_home_root.join(GEMINI_HOME_DIR)
}

pub fn is_profile_initialized(cli_home_root: &Path) -> bool {
    gemini_home_from_cli_root(cli_home_root)
        .join(GEMINI_OAUTH_FILE)
        .exists()
}

pub fn load_instance_store() -> Result<InstanceStore, String> {
    let path = instances_path()?;
    instance_store::load_instance_store(&path, GEMINI_INSTANCES_FILE)
}

pub fn save_instance_store(store: &InstanceStore) -> Result<(), String> {
    let path = instances_path()?;
    instance_store::save_instance_store(&path, GEMINI_INSTANCES_FILE, store)
}
...
pub fn get_default_gemini_cli_home_root() -> Result<PathBuf, String> {
...
pub fn get_instance_defaults() -> Result<InstanceDefaults, String> {
    ...
    let default_user_data_dir = get_default_gemini_cli_home_root()?;
    Ok(InstanceDefaults {
...
fn ensure_target_root_empty(path: &Path) -> Result<(), String> {
    ...
    if has_entries {
        let resolved_path = instance_store::display_path(path);
        return Err(format!("复制来源实例需要目标目录为空: {}", resolved_path));
    }
    Ok(())
}

fn resolve_copy_source_cli_home_root(
    store: &InstanceStore,
    copy_source_instance_id: Option<&str>,
) -> Result<PathBuf, String> {
    match copy_source_instance_id {
        Some("__default__") | None => get_default_gemini_cli_home_root(),
        Some(source_id) => {
            let source_instance = store
                ...
                .ok_or("复制来源实例不存在")?;
            Ok(PathBuf::from(&source_instance.user_data_dir))
        }
    }
}
...
pub fn create_instance(params: CreateInstanceParams) -> Result<InstanceProfile, String> {
    ...
    if use_existing_dir {
        ...
        ensure_target_root_empty(&user_dir_path)?;
        let source_root =
            resolve_copy_source_cli_home_root(&store, params.copy_source_instance_id.as_deref())?;
        let source_gemini_dir = gemini_home_from_cli_root(&source_root);
        if !source_gemini_dir.exists() {
            return Err("未找到复制来源目录，请先确保来源实例已初始化".to_string());
        }
        if !source_gemini_dir.is_dir() {
            return Err("复制来源目录非法".to_string());
        }

        fs::create_dir_all(&user_dir_path).map_err(|e| format!("创建实例目录失败: {}", e))?;
        let target_gemini_dir = gemini_home_from_cli_root(&user_dir_path);
        instance_store::copy_dir_recursive(&source_gemini_dir, &target_gemini_dir)?;
    }
