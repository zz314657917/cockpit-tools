...
const CODEBUDDY_INSTANCES_FILE: &str = "codebuddy_cn_instances.json";
...
pub fn update_default_settings(
    ...
    // CodeBuddy CN 实例不支持“跟随当前账号”，直接忽略 follow_local_account。
...
pub fn get_default_codebuddy_cn_user_data_dir() -> Result<PathBuf, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        return Ok(home.join("Library/Application Support/CodeBuddy CN"));
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        return Ok(PathBuf::from(appdata).join("CodeBuddy CN"));
    }

    #[cfg(target_os = "linux")]
    {
        let home = dirs::home_dir().ok_or("无法获取用户主目录")?;
        return Ok(home.join(".config/CodeBuddy CN"));
    }
        ...
        if user_dir_path.exists() {
            ...
            if has_entries {
                ...
                modules::logger::log_info(&format!(
                    "[CodeBuddy CN Instance] 复制来源实例需要空目录，但目标已存在: {}",
                    resolved_path
                ));
