...
fn ensure_codebuddy_cn_state_db_path(user_data_dir: &str) -> Result<PathBuf, String> {
    ...
    if let Some(default_db) =
        modules::codebuddy_cn_account::get_default_codebuddy_cn_state_db_path()
    {
            ...
            match fs::copy(&default_db, &preferred) {
                ...
                Err(err) => {
                    modules::logger::log_warn(&format!(
                        "[CodeBuddy CN Inject] 复制默认 state.vscdb 失败，改为写入新库: from={}, to={}, error={}",
                        default_db.to_string_lossy(),
                        preferred.to_string_lossy(),
                        err
                    ));
...
fn resolve_running_pid(last_pid: Option<u32>, user_data_dir: Option<&str>) -> Option<u32> {
    modules::process::resolve_codebuddy_cn_pid(last_pid, user_data_dir)
}
...
fn inject_bound_account_for_instance_start(
    ...
    let account = modules::codebuddy_cn_account::load_account(bind_id)
        .ok_or_else(|| format!("绑定账号不存在: {}", bind_id))?;
    modules::logger::log_info(&format!(
        "[CodeBuddy CN Instance] 实例启动检测到绑定账号，准备注入: bind_account_id={}, email={}, user_data_dir={}",
        bind_id, account.email, user_data_dir
    ));

    let state_db_path = ensure_codebuddy_cn_state_db_path(user_data_dir)?;
...
#[tauri::command]
pub async fn codebuddy_cn_get_instance_defaults(
) -> Result<modules::instance::InstanceDefaults, String> {
    modules::codebuddy_cn_instance::get_instance_defaults()
}

#[tauri::command]
pub async fn codebuddy_cn_list_instances() -> Result<Vec<InstanceProfileView>, String> {
...
#[tauri::command]
pub async fn codebuddy_cn_create_instance(
    name: String,
    ...
    let instance = modules::codebuddy_cn_instance::create_instance(
        modules::codebuddy_cn_instance::CreateInstanceParams {
            name,
            user_data_dir,
            working_dir: None,
...
#[tauri::command]
pub async fn codebuddy_cn_update_instance(
    instance_id: String,
    name: Option<String>,
    extra_args: Option<String>,
    ...
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = modules::codebuddy_cn_instance::get_default_codebuddy_cn_user_data_dir()?;
        let default_dir_str = default_dir.to_string_lossy().to_string();
        let updated = modules::codebuddy_cn_instance::update_default_settings(
            bind_account_id,
            extra_args,
    ...
    let instance = modules::codebuddy_cn_instance::update_instance(
        modules::codebuddy_cn_instance::UpdateInstanceParams {
            instance_id,
            name,
            working_dir: None,
            extra_args,
...
#[tauri::command]
pub async fn codebuddy_cn_delete_instance(instance_id: String) -> Result<(), String> {
    if instance_id == DEFAULT_INSTANCE_ID {
        return Err("默认实例不可删除".to_string());
    }
    modules::codebuddy_cn_instance::delete_instance(&instance_id)
}

#[tauri::command]
pub async fn codebuddy_cn_start_instance(
    instance_id: String,
) -> Result<InstanceProfileView, String> {
    modules::process::ensure_codebuddy_cn_launch_path_configured()?;
    ...
    if instance_id == DEFAULT_INSTANCE_ID {
        ...
        let pid = modules::process::start_codebuddy_cn_default_with_args_with_new_window(
            &extra_args,
            true,
        )?;
        let _ = modules::codebuddy_cn_instance::update_default_pid(Some(pid))?;
    ...
    let pid = modules::process::start_codebuddy_cn_with_args_with_new_window(
        &instance.user_data_dir,
        &extra_args,
        true,
    )?;

    let updated = modules::codebuddy_cn_instance::update_instance_after_start(&instance.id, pid)?;
...
#[tauri::command]
pub async fn codebuddy_cn_stop_instance(
    instance_id: String,
) -> Result<InstanceProfileView, String> {
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = modules::codebuddy_cn_instance::get_default_codebuddy_cn_user_data_dir()?;
...
#[tauri::command]
pub async fn codebuddy_cn_open_instance_window(instance_id: String) -> Result<(), String> {
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_settings = modules::codebuddy_cn_instance::load_default_settings()?;
